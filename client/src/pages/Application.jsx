          ...
          <table className="min-w-full bg-white border rounded-lg"> 
            ...
            <tbody>
                ...
                {jobsApplied.map((job,index)=> true ? (
                  <tr>
                    <td className="py-3 px-4 flex items-center gap-2 border-b"><img className="w-8 h-8.5" src={job.logo} />{job.company}</td>
                    ...
                    <td className="px-2 py-4 border-b">
                      <span className={`${job.status === 'Accepted' ? 'bg-green-100' : job.status === 'Rejected' ? 'bg-red-100' : 'bg-blue-100'} px-4 py-1.5 rounded-lg`}>{job.status}</span></td>
                  </tr>
                ) : (null))}
