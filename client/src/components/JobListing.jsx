...
const JobListing = () => {
  ...
  useEffect(()=>{
    ...
    const machesSearchLocation = job => searchFilter.location ===""||job.location.toLowerCase().includes(searchFilter.location.toLowerCase())
    const newFilteredJobs =jobs.slice().reverse().filter(
        job => matchesCategory(job) && matchesLocation(job) && matchesTitle(job) && machesSearchLocation(job)
    )
        ...
        <div className={showFilter ? "" : "max-lg:hidden"}>
          ...
          <ul className="space-y-4 text-gray-600">
            {JobLocations.map((location, index) => (
              <li className="flex gap-3 items-center" key={index}>
                <input className="scale-125" type="checkbox" onClick={() => handleLocationChange(location)}
